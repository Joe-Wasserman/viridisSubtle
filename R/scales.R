...
#'
#' @export
viridis_pal <- function(alpha = 1, begin = 0, end = 1, subtle = FALSE, direction = 1, option= "D") {
  function(n) {
    viridis(n, alpha, begin, end, subtle, direction, option)
  }
}
...
#'
#' @export
scale_color_viridis <- function(..., alpha = 1, begin = 0, end = 1, subtle = FALSE, direction = 1,
                                discrete = FALSE, option = "D") {
  if (!discrete & subtle) {
    warning("subtle is only applicable to discrete scales, ignoring subtle")
  }
  
  if (discrete) {
    discrete_scale("colour", "viridis", viridis_pal(alpha, begin, end, subtle, direction, option), ...)
  } else {
...
#'
#' @export
scale_fill_viridis <- function(..., alpha = 1, begin = 0, end = 1, subtle = FALSE, direction = 1,
                               discrete = FALSE, option = "D") {
  if (discrete) {
    discrete_scale("fill", "viridis", viridis_pal(alpha, begin, end, subtle, direction, option), ...)
  } else {
